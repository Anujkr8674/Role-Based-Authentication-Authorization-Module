...
if (isset($_SESSION['manager_id']) || isset($_COOKIE['manager_auth_token'])) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['login'])) {
    ...
    $stmt = $conn->prepare("SELECT id, manager_id, name, email, password FROM `manager` WHERE email = ? OR manager_id = ?");
    ...
    if ($row = $result->fetch_assoc()) {
        if (password_verify($password, $row['password'])) {
            ...
            $last_login_check = $conn->query("SHOW COLUMNS FROM `manager` LIKE 'last_login'");
            ...
            if ($last_login_exists) {
                $update_stmt = $conn->prepare("UPDATE `manager` SET auth_token = ?, token_expiry = ?, last_login = NOW() WHERE id = ?");
                $update_stmt->bind_param("ssi", $token, $token_expiry, $row['id']);
            } else {
                $update_stmt = $conn->prepare("UPDATE `manager` SET auth_token = ?, token_expiry = ? WHERE id = ?");
                $update_stmt->bind_param("ssi", $token, $token_expiry, $row['id']);
            }
            $update_stmt->execute();
            $update_stmt->close();
            ...
            setcookie('manager_auth_token', $token, buildCookieOptions(time() + (7 * 24 * 60 * 60), $is_secure));
        ...
        $error_message = "No manager found with this Email/Manager ID.";
    }
...
<div class="container1">
    ...
    <div class="main-title">Login to Manager Account</div>
    <div class="subtitle">Access your manager profile</div>
    ...
    <form class="form" method="post" action="">
        <div class="input-box">
            <label class="form-label" for="login_id">Email or Manager ID</label>
            <input type="text" id="login_id" name="login_id" placeholder="Enter your email or manager ID" required>
        </div>
        <div class="input-box">
    ...
    <div style="text-align:center; margin-top:5px; margin-bottom:10px;">
        <a href="manager-forgot-password.php" style="color:#4f7cff; text-decoration:none; font-size:1rem;">Forgot password?</a>
    </div>

    <div class="register-link">
        Don't have an account? <a href="manager-register.php">Register</a>
    </div>
</div>
<br/><br/>
<script>
