...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['change_password'])) {
    ...
    if ($current_password === '' || $new_password === '' || $confirm_password === '') {
        ...
        $password_stmt = $conn->prepare("SELECT password FROM `manager` WHERE manager_id = ?");
        $password_stmt->bind_param('s', $_SESSION['manager_id']);
        ...
        if (!$password_row || !password_verify($current_password, $password_row['password'])) {
            ...
            $update_password_stmt = $conn->prepare("UPDATE `manager` SET password = ? WHERE manager_id = ?");
            $update_password_stmt->bind_param('ss', $new_hashed_password, $_SESSION['manager_id']);
...
$page_subtitle = 'Update manager password securely';
