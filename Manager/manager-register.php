...
<head>
    ...
    <script>
        function generateManagerId() { return Math.floor(100000 + Math.random() * 900000).toString(); }
        function validatePassword(pw) { return /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[^A-Za-z0-9]).{8,16}$/.test(pw); }
    ...
    <div class="container1">
        <div id="success-popup" class="success-popup">
            <div class="success-popup-card">
                ...
                <p id="success-popup-text">Your manager account has been created successfully. Redirecting to sign in...
                </p>
            </div>
        </div>
        ...
        <div class="main-title">Create Manager Account</div>
        <div class="subtitle">Register to access manager dashboard</div>
        ...
        <div class="input-box" id="otp-section" style="display:none; position:relative;">
            <input type="text" id="otp" name="otp" placeholder="Enter OTP" maxlength="6" inputmode="numeric"
                pattern="[0-9]*" autocomplete="one-time-code">
            <button class="verify-btn" id="verify-otp-btn"
                style="position:absolute; right:25px; top:50%; transform:translateY(-50%);">Verify OTP</button>
        </div>
        ...
        <div id="password-section" style="display:none;">
            <form id="registerForm" autocomplete="off">
                <div class="input-box password-box">
                    ...
                    <div class="message" style="font-size:0.95rem; color:#23238b;">Password must be 8-16 characters and
                        include uppercase, lowercase, digit, and special character.</div>
                </div>
                <div class="input-box password-box">
        ...
        <div class="register-link">
            Already have an account? <a href="manager-signin.php">Sign In</a>
        </div>
        ...
        <script>
            ...
            function showRegistrationSuccessPopup(managerId) {
                const popup = document.getElementById('success-popup');
                const text = document.getElementById('success-popup-text');
                text.innerText = `Your manager account has been created successfully. Manager ID: ${managerId}. Redirecting to sign in...`;
                popup.classList.add('show');
                setTimeout(() => {
                    window.location.href = 'manager-signin.php';
                }, 2200);
            }
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                window.generatedManagerId = generateManagerId();
                ...
                otpForm.addEventListener('submit', async function (e) {
                    ...
                    const checkRes = await fetch('manager-check-email.php', {
                        method: 'POST',
                    ...
                    const res = await fetch('manager-send-otp.php', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                ...
                verifyOtpBtn.addEventListener('click', async function (e) {
                    ...
                    const res = await fetch('manager-verify-otp.php', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                ...
                registerForm.addEventListener('submit', async function (e) {
                    ...
                    const managerId = window.generatedManagerId || generateManagerId();
                    ...
                    try {
                        const res = await fetch('manager-register-user.php', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                            body: `manager_id=${encodeURIComponent(managerId)}&name=${encodeURIComponent(name)}&email=${encodeURIComponent(email)}&password=${encodeURIComponent(password)}`
                        });
                        const data = await res.json();

                        if (data.success) {
                            window.generatedManagerId = data.manager_id;
                            showRegistrationSuccessPopup(data.manager_id);
                        } else {
