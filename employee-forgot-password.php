...
if (isset($_POST['send_otp']) || isset($_POST['resend_otp'])) {
    ...
    if ($stmt->num_rows == 0) {
        ...
        try {
            ...
            $mail->isHTML(true);
            $mail->Subject = 'Employee Account - OTP for Password Reset';
            $mail->Body = "
                <div style='font-family: Arial, sans-serif; font-size: 16px; color: #333;'>
                    ...
                    <p><strong>Note:</strong> This OTP is valid for the next 5 minutes only. 
                    ...
                    <p>Best Regards,<br><strong>Anuj Kumar</strong></p>
                </div>";
            $mail->send();
...
if (isset($_POST['verify_otp'])) {
    $otp = trim($_POST['otp']);
    $expired = !isset($_SESSION['fp_otp_time']) || (time() - $_SESSION['fp_otp_time'] >= 300);
    if (isset($_SESSION['fp_otp']) && $otp == $_SESSION['fp_otp'] && !$expired) {
...
if (isset($_POST['reset_password'])) {
    if (!isset($_SESSION['fp_verified']) || !$_SESSION['fp_verified']) {
        ...
        if ($password !== $confirm) {
            ...
            if ($stmt->execute()) {
                $message = "Password reset successful! You can now <a href='employee-signin.php'>login</a>.";
                unset($_SESSION['fp_otp'], $_SESSION['fp_email'], $_SESSION['fp_otp_time'], $_SESSION['fp_verified']);
                $step = 4;
                echo "<script>alert('Password reset successful! Please login.'); setTimeout(function(){ window.location.href = 'employee-signin.php'; }, 1000);</script>";
            } else {
...
<body>
    ...
    <div class="container1">
        <div class="main-title">Employee Forgot Password</div>
        <?php if ($message): ?>
            <div class="message<?php echo ($step == 2 && strpos($message, 'Invalid') !== false) ? ' error' : ''; ?>">
                <?php echo $message; ?></div><?php endif; ?>
        <?php if ($step == 1): ?>
            <form method="post">
                <div class="input-box"><label for="email">Enter your registered email</label><input type="email"
                        name="email" id="email" required></div>
                <button class="btn" type="submit" name="send_otp">Send OTP</button>
            </form>
        <?php elseif ($step == 2): ?>
            <form method="post" id="otpForm">
                <div class="input-box"><label for="otp">Enter OTP sent to your email</label><input type="text" name="otp"
                        id="otp" maxlength="6" pattern="\d{6}" required></div>
                <div id="otp-timer"></div>
                <button class="btn" type="submit" name="verify_otp">Verify OTP</button>
            </form>
            <form method="post" style="margin-top:-10px;">
                <input type="hidden" name="email"
                    value="<?php echo htmlspecialchars($_SESSION['fp_email'] ?? $_POST['email'] ?? ''); ?>">
                <button class="resend-btn" type="submit" name="resend_otp" id="resend-otp-btn" disabled>Resend OTP</button>
            </form>
            <script><?php if (isset($_SESSION['fp_otp_time'])): ?>startOTPTimer(<?php echo $_SESSION['fp_otp_time']; ?>); <?php endif; ?></script>
        <?php elseif ($step == 3): ?>
            <form method="post">
                <div class="input-box"><label for="password">New Password</label><input type="password" name="password"
                        id="password" required></div>
                <div class="input-box"><label for="confirm_password">Confirm New Password</label><input type="password"
                        name="confirm_password" id="confirm_password" required></div>
